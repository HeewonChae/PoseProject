...
namespace Pose_sports_statistics.Controls
{
	public partial class FootballFixtureList : System.Web.UI.UserControl
	{
		...
		public void DDLBindData()
		{
			...
			// 종료된 경기 필터링
			var exclusive_finished = fixtures.Where(elem => elem.EventDate > DateTime.Now.AddHours(-2));
		...
		public IQueryable<dynamic> GetFixtures([RouteData] string countryName)
		{
			...
			{
				filteredByData = fixtures.Where(elem => elem.EventDate > DateTime.Now.AddHours(-2));
			}
			else
			{
				filteredByData = fixtures.Where(elem => elem.EventDate > DateTime.Now.AddHours(-2) && elem.League.Country.Equals(countryName));
			}

			// step2. 데이터를 리그, 시작시간으로 그룹화
			var group_query = filteredByData.GroupBy(elem => new { League = elem.LeagueID, StartTime = elem.EventDate });
