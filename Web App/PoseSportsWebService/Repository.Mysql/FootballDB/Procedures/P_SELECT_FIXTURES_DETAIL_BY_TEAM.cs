...
{
    public class P_SELECT_FIXTURES_DETAIL_BY_TEAM : MysqlQuery<P_SELECT_FIXTURES_DETAIL_BY_TEAM.Input, P_SELECT_FIXTURES_DETAIL_BY_TEAM.Output>
    {
        ...
        public override void BindParameters()
        {
            ...
            FinishedFixturesQuery = $"{DB_FootballFixtureDetail.SelectQuery} " +
                $"WHERE ( f.{nameof(Fixture.home_team_id)} = {_input.TeamId} AND f.{nameof(Fixture.match_time)} <= @CurDate ) " +
                $"OR ( f.{nameof(Fixture.away_team_id)} = {_input.TeamId} AND f.{nameof(Fixture.match_time)} <= @CurDate ) " +
