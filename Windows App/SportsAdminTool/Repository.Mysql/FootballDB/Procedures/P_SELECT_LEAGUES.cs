...
{
    public class P_SELECT_LEAGUES : MysqlQuery<P_SELECT_LEAGUES.Input, IEnumerable<Table.League>>
    {
        public struct Input
        {
            string _where;
        ...
        public override IEnumerable<Table.League> OnQuery()
        {
            DapperFacade.DoWithDBContext(
                    null,
                    (Contexts.FootballDB footballDB) =>
                    {
                        _output = footballDB.QuerySQL<Table.League>($"SELECT * FROM league {_input.Where} {_input.GroupBy} {_input.OrderBy}");
                    },
                    this.OnError);
