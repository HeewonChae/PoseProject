...
{
    public class P_SELECT_STANDINGS : MysqlQuery<short, IEnumerable<Table.Standing>>
    {
        ...
        public override IEnumerable<Table.Standing> OnQuery()
        {
            DapperFacade.DoWithDBContext(
                    null,
                    (Contexts.FootballDB footballDB) =>
                    {
                        _output = footballDB.QuerySQL<Table.Standing>($"SELECT * FROM standing WHERE league_id = {_input}");
                    },
                    this.OnError);
