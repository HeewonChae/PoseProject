...
{
	public class P_SELECT_FIXTURES : MysqlQuery<P_SELECT_FIXTURES.Input, IEnumerable<Table.Fixture>>
	{
		public struct Input
		{
		...
		public override IEnumerable<Table.Fixture> OnQuery()
		{
			DapperFacade.DoWithDBContext(
					null,
					(Contexts.FootballDB footballDB) =>
					{
						_output = footballDB.QuerySQL<Table.Fixture>($"SELECT * FROM fixture {_input.Where} {_input.GroupBy} {_input.OrderBy}");
					},
					this.OnError);
