...
{
    public class P_SELECT_COVERAGE : MysqlQuery<short, Table.Coverage>
    {
        ...
        public override Table.Coverage OnQuery()
        {
            DapperFacade.DoWithDBContext(
                    null,
                    (Contexts.FootballDB footballDB) =>
                    {
                        _output = footballDB.QuerySQL<Table.Coverage>($"SELECT * FROM coverage where league_id = {_input}").FirstOrDefault();
                    },
                    this.OnError);
